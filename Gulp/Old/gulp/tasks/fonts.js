...
export const fontsStyle = () => {
	...
	fs.readdir( app.path.build.full.fonts, function( _, fontsFiles ) {
		if (fontsFiles) {
			if (!fs.existsSync( fontsFile )) {
				...
				for (let i = 0; i < fontsFiles.length; i++) {
					...
					if (newFileOnly !== fontFileName) {
						...
						else if (fontWeight.toLowerCase() === "thin") fontWeight = 900;
						else fontWeight = 400;
				...
				console.log( "Файл css/fonts.css уже существует. Для обновления удалите его. Сделано с целью: не сбивать ручные настройки файла постоянным удалением при запуске сборки." );
			}
		}
	});
...
export const notify = () => {
	return console.log( "Замечены изменения в папке fonts. Перезагрузите сборку. Перед перезагрузкой нужно удалить файл css/fonts.css!" );
}